...
import {Check, ChevronDown, ChevronUp, ExternalLink, History, Pencil, Replace, Search, X} from "lucide-react";
import {GLOBAL_PLACEHOLDER_REGEX, toPlaceholderName} from "@/lib/placeholder/placeholderUtil";
import {Label} from "@radix-ui/react-label";
import {Card, CardContent, CardFooter} from "@/components/ui/card.tsx";
import {cn} from "@/lib/utils.ts";
import {useAutoAnimate} from "@formkit/auto-animate/react";
import type {BrowserStorageApi, BrowserTab, BrowserTabApi} from "@/lib/browser/BrowserTabApi.ts";
import type {HistoryItem} from "@/popup/PopupStore.ts";
import {useHistoryStore} from "@/popup/PopupStore.ts";
import {InputWithAdornments} from "@/components/InputWithAdornments.tsx";
import {Select, SelectContent, SelectItem, SelectTrigger, SelectValue} from "@/components/ui/select.tsx";

function Popup({browserTabApi}: {
    browserTabApi: BrowserTabApi;
    browserStorageApi: BrowserStorageApi;
}) {
    const [tab, setTab] = useState<BrowserTab | null>(null);
    const [plainUrl, setPlainUrl] = useState<string>('');
    const [urlParts, setUrlParts] = useState<[boolean, string][]>([]);
    const [placeholderValueMap, setPlaceholderValueMap] = useState<Record<string, string>>({});

    useEffect(() => {
        (async () => {
            const [activeTab] = await browserTabApi.queryTabs({active: true, currentWindow: true});
            if (!activeTab) {
            ...
            const placeholderValueMap: Record<string, string> = urlParts
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        for (const [placeholder, val] of Object.entries(placeholderValueMap)) {
            resolvedUrl = resolvedUrl.replace(new RegExp(placeholder, 'g'), encodeURIComponent(val));
        }

        await browserTabApi.updateTab(tab!.id!, {url: resolvedUrl});
        window.close();
    };
    ...
    return (
        <form
            ...
            className="w-[500px] max-h-[600px] flex flex-col py-4 gap-4 overflow-hidden"
        >

            {/* Title */}
            <div className="flex flex-row gap-2 mx-4">
                <Replace/>
                <span className="text-xl font-bold">PlaceholdURL</span>
            </div>
